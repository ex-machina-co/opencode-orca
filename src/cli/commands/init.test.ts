...
describe(() => {
  ...
  test('creates .opencode directory and orca.json', async () => {
    await init()
  ...
  test('creates valid JSON config', async () => {
    ...
    const content = readFileSync(join(TEST_DIR, '.opencode', 'orca.json'), 'utf-8')
  ...
  test('does not overwrite existing config without --force', async () => {
    ...
    writeFileSync(join(TEST_DIR, '.opencode', 'orca.json'), '{"custom": "config"}')
